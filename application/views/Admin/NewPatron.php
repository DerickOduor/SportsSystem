             ...
             <div class="">
              <div class="row">
                <div class="col-4 d-flex">
                  <div class="container">
              <form action="http://localhost/sportssystem/index.php/Admin/SubmitPatron" method="post" class="form" id="patronForm">
                  <div id="warning"></div>  
                <div class="form-group">
                    <label class="control-label">Name</label>
                    <input type="text" name="name" id="name" class="form-control" placeholder="Name of Patron" required />
                </div>  
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="confirm" id="confirm" placeholder="Re-enter password" class="form-control" required />
                </div>-->
                <div class="form-group">
                    <label class="control-label">Phone</label>
                    <input type="text" name="phone" id="phone" placeholder="0712 345 678"class="form-control" required />
                </div>
                <div class="form-group">
...
function closeNav() {
    ...
    $(document).ready(function() {
        ...
        $('.btn').click(function(e){
            //e.preventDefault();
            /*$('#patronForm').on('submit',function(e){
                e.preventDefault();
            });*/
            var phone=$('#phone').val();
            ...
            if(!(/^[a-zA-Z]*$/).test($('#name').val())){
                $('#warning').html('<div class="alert alert-danger"role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>Letters only no name spaces in <b>Name</b>!</div>');
                $('#name').focus();
            }else if(!(/([\w\-]+\@[\w\-]+\.[\w\-]+)/).test($('#email').val())){
                $('#warning').html('<div class="alert alert-danger"role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>Invalid E-mail Address!</div>');
                $('#email').focus();
            }else if((password.length<6||password.length>15)){
                $('#warning').html('<div class="alert alert-danger"role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>Password must be between 6-15 characters!</div>');
                $('#password').focus();
            }else if($('#password').val()!==$('#confirm').val()){
                $('#warning').html('<div class="alert alert-danger"role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>These passwords do not match!</div>');
                $('#password').focus();
                $('#confirm').focus();
            }else if((!(/^[0-9]*$/).test($('#phone').val())||phone.length!==10)){
                $('#warning').html('<div class="alert alert-danger"role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="sr-only">Error:</span>Invalid Phone no. 10 digits only!</div>');
                $('#phone').focus();
            }else{
                $('#patronForm').submit();
            }
